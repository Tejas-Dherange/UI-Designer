...
export const PropertyPanel: React.FC<PropertyPanelProps> = ({ onClose }) => {
  ...
  // Function to handle hover effect type change
  const handleHoverEffectChange = (value: string) => {
    ...
    const updatedComponent = {
      ...selectedComponent,
      hoverEffect: value,
      // Initialize hoverStyles if they don't exist
      hoverStyles: selectedComponent.hoverStyles || {}
    };
    
    // Set default hover styles based on the selected effect
    if (value === 'hoverBg') {
      updatedComponent.hoverStyles = {
        ...updatedComponent.hoverStyles,
        backgroundColor: '#f0f0f0'
      };
    } else if (value === 'hoverShadow') {
      updatedComponent.hoverStyles = {
        ...updatedComponent.hoverStyles,
        boxShadow: '0 4px 6px rgba(0,0,0,0.1)'
      };
    } else if (value === 'hoverBorder') {
      updatedComponent.hoverStyles = {
        ...updatedComponent.hoverStyles,
        borderColor: '#000000'
      };
    ...
    // Update component in the store
    updateComponent(selectedComponentId, updatedComponent);
  };

  // Function to handle changes to specific hover styles
          ...
          <div className="p-3 space-y-3">
            ...
            <div>
              ...
              <input 
                type="number" 
                className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                value={selectedComponent.props.style.rotate?.replace('deg', '') || 0}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input 
                  type="number"
                  className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                  value={selectedComponent.props.style.skewX?.replace('deg', '') || 0}
              ...
              <div>
                ...
                <input 
                  type="number"
                  className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                  value={selectedComponent.props.style.skewY?.replace('deg', '') || 0}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input 
                  type="number"
                  className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                  value={selectedComponent.props.style.translateX?.replace('px', '') || 0}
              ...
              <div>
                ...
                <input 
                  type="number"
                  className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                  value={selectedComponent.props.style.translateY?.replace('px', '') || 0}
        ...
        {expandedSections.appearance && (
          <div className="p-3 space-y-3">
            ...
            <div>
              ...
              <select 
                className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                value={selectedComponent.hoverEffect || 'none'}
            ...
            {/* Hover Effect Settings (conditionally rendered based on selected effect) */}
            {selectedComponent.hoverEffect === 'hoverBg' && (
              <div>
                <label className="block text-xs text-gray-500 mb-1">Hover Background Color</label>
                <input 
                  type="color" 
                  className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                  value={selectedComponent.hoverStyles?.backgroundColor || '#f0f0f0'}
                  onChange={(e) => handleHoverStyleChange('backgroundColor', e.target.value)}
                />
              </div>
            )}
            
            {selectedComponent.hoverEffect === 'hoverShadow' && (
              <div>
                <label className="block text-xs text-gray-500 mb-1">Hover Shadow</label>
                <select 
                  className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                  value={selectedComponent.hoverStyles?.boxShadow || '0 4px 6px rgba(0,0,0,0.1)'}
                  onChange={(e) => handleHoverStyleChange('boxShadow', e.target.value)}
                >
                  <option value="0 4px 6px rgba(0,0,0,0.1)">Small</option>
                  <option value="0 8px 15px rgba(0,0,0,0.1)">Medium</option>
                  <option value="0 15px 30px rgba(0,0,0,0.15)">Large</option>
                </select>
              </div>
            )}
            
            {selectedComponent.hoverEffect === 'hoverBorder' && (
              <div>
                <label className="block text-xs text-gray-500 mb-1">Hover Border Color</label>
                <input 
                  type="color" 
                  className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
                  value={selectedComponent.hoverStyles?.borderColor || '#000000'}
